...
##-----------------------------------------------------------------------------
# 1. Load and clean sample data - all samples AND passive only
##-----------------------------------------------------------------------------

setwd("~/Rabies SES/")

# Load samples SES data and remove all SES = NA
samps_all <- read_rds("data/AQPsamples_SES_6.24.2024.rds") %>%
...
# Load only passive samples
samps_pass <- read_rds("data/AQPsamples_passive_10.25.2024.rds") %>%
  mutate(twoyear = case_when(year %in% 2015:2016 ~ "2015-2016",
                             year %in% 2017:2018 ~ "2017-2018",
                             year %in% 2019:2020 ~ "2019-2020",
                             year %in% 2021:2022 ~ "2021-2022"))
...
positivity_allyears <- samps_pass %>%
...
# Combine 2021-22 data from active + passive and passive only summary data
all_2021 <- filter(pos2_all, twoyear == "2021-2022") %>%
  mutate(mechanism = "Active + Passive")
pass_2021 <- filter(pos2_pass, twoyear == "2021-2022") %>%
  mutate(mechanism = "Passive")
combo <- bind_rows(all_2021, pass_2021)

##-----------------------------------------------------------------------------
# 2. Plot positivity across all years 
##-----------------------------------------------------------------------------

# Bar plot with margin of error bars - all years
pdf("figures/R_output/barplot_positivity_me_passiveonly.pdf", width = 7, height = 4.5)
...
ggplot(positivity_allyears, aes(x = SES, y = positivity, fill = SES)) +
  ...
  scale_fill_manual(values = c("#2c7bb6", "#abd9e9", "#ffffbf", "#fdae61", 
                               "#d7191c", "grey")) +
  scale_y_continuous(expand = c(0,0), limits = c(0, 39))  +
  my_econ_theme() +    
  xlab("Year") +
...
pdf("figures/R_output/barplot_twoyearpositivity_me_passiveonly.pdf", width = 7, height = 4.5)
ggplot(combo, aes(x = mechanism, y = positivity, fill = SES)) +
